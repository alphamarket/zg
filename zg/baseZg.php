...
if(!defined("ZG_ROOT"))
{
    ...
    defined("ZG_ROOT") ||  define("ZG_ROOT", dirname(dirname(__FILE__)));
    # defines zinux cache dir
    defined("Z_CACHE_ROOT") ||  define("Z_CACHE_ROOT", dirname(ZG_ROOT)."/zinux");
    ...
    defined("ZG_VERSION") || define("ZG_VERSION","2.0.0");
...
abstract class baseZg extends \zinux\baseZinux
{
     ...
     * @param \zg\vendors\status $s the status object
     */
    public function SaveStatus(\zg\vendors\status $s)
    {
        unset($s->hs);
        $s->hs = \zinux\kernel\security\hash::Generate(serialize($s),1,1);
     ...
     * @return string
     */
    public function convert_to_relative_path($path, $project_dir = ".", \zg\vendors\status $s = null)
    {
        if(!$s)
