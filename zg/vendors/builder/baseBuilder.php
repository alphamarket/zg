<?php
namespace zg\vendors\builder;

/**
 * base class for \zg\operators\build class
 */
abstract class baseBuilder extends \zg\operators\baseOperator
{
    /**
     ...
     * @return \zg\vendors\status created status object 
    ...
    public function creatVirtualStatusFile($project_path)
    {
        ...
        $s = new \zg\vendors\status;
        # set project parent item
        $parent = new \zg\vendors\item(dirname($project_path), dirname($project_path));
        # init status project item
        $s->project = new \zg\vendors\item("project", $project_path ,$parent);
        # indicate a step
        $this->step_show();
        # create the hash sum
    ...
    protected function fetchApplication()
    {
        ...
        if(!($this->app = \zinux\kernel\utilities\fileSystem::resolve_path($this->app)))
        {
            ...
            $this->log[] = new \zg\vendors\item("Application directory not found","Skipping processing application directory.");
            # return
            return;
        }
        # indicate a step
        ...
        foreach(glob("{$this->app}/*") as $file)
        {
            ...
            # create new app item
            $app = new \zg\vendors\item($name, $file, $this->s->project);
    ...
    protected function fetchModules()
    {
        ...
        foreach(glob("{$this->modules}/*", GLOB_ONLYDIR) as $module)
        {
            ...
            if(preg_match("#\w+module$#i", $module))
            {
                ...
                $module = new \zg\vendors\item("{$name}", "{$this->s->modules->meta->path}/{$name}", $this->s->modules->meta);
                ...
                if(($bootstrap = \zinux\kernel\utilities\fileSystem::resolve_path($module->path.DIRECTORY_SEPARATOR.$bs)))
                {
                    ...
                    $moduleBootstrap = new \zg\vendors\item(basename($bs, ".php"), $bootstrap, $module);
            ...
            else
            {
                ...
                $this->step_show(1, "x", self::getColor(self::hiRed));
                # flag it as error
                $this->log[] = new \zg\vendors\item("Folder '$module' skipped", "Didn't match with standrad module folder pattern.");
            }
        }
        # indicate a step
        $this->step_show();
    ...
    protected function fetchController()
    {
        ...
        foreach($this->s->modules->collection as $name => $module)
        {
            ...
            if(!($cp = \zinux\kernel\utilities\fileSystem::resolve_path("{$module->path}/controllers")))
            {
                $this->step_show(1, "x", self::getColor(self::hiRed));
                # flag it as error
                $this->log[] = new \zg\vendors\item("Controllers folder not found at '{$module->path}'", "");
            ...
            foreach(glob("$cp/*") as $file)
            {
                ...
                if(is_file($file) && preg_match("#\w+controller.php$#i", $file))
                {
                    ...
                    {  
                        $this->step_show(1, "x", self::getColor(self::hiRed));
                        # flag it as error
                        $this->log[] = new \zg\vendors\item("File '$file' skipped", "Couldn't open the file.");
                        # continue with others
                        continue;
                    }
                    # indicate a step
                    ...
                    if(!class_exists($class))
                    {
                        ...
                        $this->log[] = new \zg\vendors\item("Controller '$file' found, but relative class '$class' not found","");  
                        # continue with others
                        continue;
                    }
                    # indicate a step
                    ...
                    $controller = new \zg\vendors\Item($name, $file, $module);
                ...
                elseif(is_file($file))
                {
                    $this->step_show(1, "x", self::getColor(self::hiRed));
                    # flag it as error
                    $this->log[] = new \zg\vendors\item("File '$file' skipped","Didn't match with standard controller file pattern.");
                }
            }
            # indicate a step
            $this->step_show();
    ...
    protected function fetchAction()
    {
        ...
        foreach($this->s->modules->collection as $module)
        {
            ...
            if(!isset($module->controller))
            {
                $this->step_show(1, "x", self::getColor(self::hiRed));
                # flag it as error
                $this->log[] = new \zg\vendors\item("No controller found in {$module->name}", "");
                # continue with others
                continue;
            }
            # indicate a step
                ...
                foreach(get_class_methods($class) as $method)
                {
                    ...
                    if(preg_match("#\w+action#i", $method))
                    {
                        ...
                        if(!is_callable(array($class, $method)))
                        {
                            $this->step_show(1, "x", self::getColor(self::hiRed));
                            # flag it as error
                            $this->log[] = new \zg\vendors\item("Method '$method' is not callable", "In class '$class' method '$method' is not callable!");
                            # continue with others
                            continue;
                        }
                        # create new action item
                        $action = new \zg\vendors\item($method, $method, $controller);
    ...
    public function fetchModels()
    {
        ...
        foreach($this->s->modules->collection as $name => $module)
        {
            ...
            if(!($mp = \zinux\kernel\utilities\fileSystem::resolve_path($module->path."/models")))
            {
                $this->step_show(1, "x", self::getColor(self::hiRed));
                # flag it as error
                $this->log[] = new \zg\vendors\item("Models folder not found at '{$module->path}'", "");
                # continue with others
                continue;
            }
            # indicate a step
            ...
            foreach(glob("$mp/*") as $file)
            {
                ...
                if(is_file($file) && preg_match("#\w+\b(.php)\b$#i", $file))
                {
                    ...
                    if(!class_exists($class))
                    {
                        $this->step_show(1, "x", self::getColor(self::hiRed));
                        # flag it as error
                        $this->log[] = new \zg\vendors\item("Model '$file' found, but relative class '$class' not found","");
                        # contiue with others
                        continue;
                    }
                    # indicate a step
                    $this->step_show();
                    # create a model item
                    $model = new \zg\vendors\Item($name, $file, $module);
    ...
    public function fetchHelpers()
    {
        ...
        foreach($this->s->modules->collection as $name => $module)
        {
            ...
            if(!($hp = \zinux\kernel\utilities\fileSystem::resolve_path($module->path."/views/helper")))
            {
                $this->step_show(1, "x", self::getColor(self::hiRed));
                # flag it as error
                $this->log[] = new \zg\vendors\item("Helpers folder not found at '{$module->path}/views/helper'", "");
                # continue with others
                continue;
            }
            # indicate a step
            ...
            foreach(glob("$hp/*") as $file)
            {
                ...
                if(is_file($file) && preg_match("#\w+\b(.php)\b$#i", $file))
                {
                    ...
                    $this->step_show();
                    # create helper item
                    $helper = new \zg\vendors\Item($name, $file, $module);
    ...
    public function fetchLayouts()
    {
        ...
        foreach($this->s->modules->collection as $name => $module)
        {
            ...
            if(!($lp = \zinux\kernel\utilities\fileSystem::resolve_path($module->path."/views/layout")))
            {
                $this->step_show(1, "x", self::getColor(self::hiRed));
                # flag it as error
                $this->log[] = new \zg\vendors\item("Layouts folder not found at '{$module->path}/views/layout'", "");
                # continue with others
                continue;
            }
            # indicate a step
            ...
            foreach(glob("$lp/*") as $file)
            {
                ...
                if(is_file($file) && preg_match("#\w+layout.phtml$#i", $file))
                {
                    ...
                    $layout = new \zg\vendors\Item($name, $file, $module);
                ...
                elseif(is_file($file))
                {
                    $this->step_show(1, "x", self::getColor(self::hiRed));
                    # flag it as error
                    $this->log[] = new \zg\vendors\item("File '$file' skipped","Didn't match with standard layout file pattern.");
                }
                # indicate a step
                $this->step_show();
            }
        ...
        foreach($this->s->modules->collection as $name => $module)
        {
            ...
            foreach($module->controller  as $cname => $controller)
            {
                ...
                if(!($vp = \zinux\kernel\utilities\fileSystem::resolve_path($module->path."/views/view/$name")))
                {
                    $this->step_show(1, "x", self::getColor(self::hiRed));
                    # flag it as error
                    $this->log[] = new \zg\vendors\item("Views folder not found at '{$module->path}/views/view/$name'", "");
                    # continue with others
                    continue;
                }
                # indicate a step
                ...
                foreach(glob("$vp/*") as $file)
                {
                    ...
                    if(is_file($file) && preg_match("#\w+view.phtml$#i", $file))
                    {
                        ...
                        $view = new \zg\vendors\Item($name, $file, $controller);
                    ...
                    elseif(is_file($file))
                    {
                        $this->step_show(1, "x", self::getColor(self::hiRed));
                        # flag it as error
                        $this->log[] = new \zg\vendors\item("File '$file' skipped","Didn't match with standard view file pattern.");
                    }
                    # indicate a step
                    $this->step_show();
                }
